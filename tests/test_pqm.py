...
def test_pass(return_type, dist, use_pytorch, z_score_norm, num_refs, ndim):
    new = []
    for _ in range(50):
...
@pytest.mark.parametrize("num_refs", [20, 100])
@pytest.mark.parametrize("ndim", [1, 50])
def test_fail(return_type, dist, use_pytorch, num_refs, ndim):
    new = []
    for _ in range(50):
        y_samples = _get_dist(dist, (500, ndim))
        ...
        new.append(pqm(x_samples, y_samples, num_refs=num_refs, return_type=return_type))
...
def test_fracs(return_type, dist, use_pytorch, x_frac, gauss_frac):
    ...
    val = pqm(x_samples, y_samples, x_frac=x_frac, gauss_frac=gauss_frac, return_type=return_type)
    if return_type == "p_value":
        assert val < 1e-3
    else:
        assert (val / (100 - 1)) > 1.5
